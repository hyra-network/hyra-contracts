...
describe("TokenVesting - Simple Tests", function () {
    ...
    async function deploySimpleFixture() {
        ...
        const tokenProxy = await proxyDeployer.deployProxy.staticCall(await tokenImpl.getAddress(), await proxyAdmin.getAddress(), tokenInit, "TEST");
        await (await proxyDeployer.deployProxy(await tokenImpl.getAddress(), await proxyAdmin.getAddress(), tokenInit, "TEST")).wait();
