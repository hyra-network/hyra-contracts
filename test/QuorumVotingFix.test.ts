...
describe("Quorum Voting Fix - Critical Bug Test", function () {
    ...
    describe("Critical Bug: Against votes counted as For votes", function () {
        ...
        it("Should correctly calculate quorum with only AGAINST votes (should NOT reach quorum)", async function () {
            ...
            // Verify vote counts
            expect(againstVotes).to.equal(ethers.parseEther("2000000")); // 2M against
            expect(forVotes).to.equal(0); // No for votes
            expect(abstainVotes).to.equal(0); // No abstain votes
            ...
            expect(finalState).to.equal(3n, "Proposal should be defeated when only against votes");
        });
        ...
        it("Should correctly calculate quorum with FOR and ABSTAIN votes (should reach quorum)", async function () {
            ...
            expect(forVotes).to.equal(ethers.parseEther("2000000")); // 2M for
            expect(abstainVotes).to.equal(ethers.parseEther("1000000")); // 1M abstain
            ...
            expect(finalState).to.equal(4n, "Proposal should succeed with for + abstain votes");
        });
        ...
        it("Should NOT count AGAINST votes toward quorum (critical fix verification)", async function () {
            ...
            console.log("  For votes:", ethers.formatEther(forVotes));
            console.log("  Abstain votes:", ethers.formatEther(abstainVotes));
            console.log("  Quorum votes (for + abstain):", ethers.formatEther(quorumVotes));
            console.log("  Required quorum:", ethers.formatEther(requiredQuorum));

            // CRITICAL ASSERTION: Quorum votes should NOT include against votes
            expect(quorumVotes).to.equal(0n, "Quorum votes should not include against votes");
            expect(quorumVotes).to.be.lessThan(requiredQuorum, "Quorum should not be reached");

            // Verify against votes are not counted
            expect(againstVotes).to.be.greaterThan(0n, "Should have against votes");
            expect(againstVotes).to.be.greaterThan(requiredQuorum, "Against votes exceed quorum");
            expect(quorumVotes).to.equal(0n, "But quorum votes should still be 0");
        ...
        it("Should handle mixed voting correctly", async function () {
            ...
            expect(quorumVotes).to.equal(
                ethers.parseEther("2500000"), // 2M for + 500K abstain
        ...
        it("Edge case: Quorum barely reached with for + abstain", async function () {
            ...
            console.log("  For votes:", ethers.formatEther(forVotes));
            console.log("  Quorum votes:", ethers.formatEther(quorumVotes));
            console.log("  Required quorum:", ethers.formatEther(requiredQuorum));
            ...
            expect(finalState).to.equal(4n, "Should succeed at exact quorum");
    ...
    describe("Regression tests - Ensure fix doesn't break normal voting", function () {
        it("Should still count FOR votes correctly", async function () {
            ...
            expect(forVotes).to.equal(
                ethers.parseEther("3000000"), // 2M + 1M
        ...
        it("Should still count ABSTAIN votes correctly", async function () {
            ...
            expect(abstainVotes).to.equal(
                ethers.parseEther("1500000"), // 1M + 500K
