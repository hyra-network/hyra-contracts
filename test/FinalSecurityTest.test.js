...
describe("Final Security Test - HNA-01 Resolution", function () {
    ...
    async function deployLegacyTokenFixture() {
        ...
        const legacyProxy = await proxyDeployer.deployProxy.staticCall(await tokenImpl.getAddress(), await proxyAdmin.getAddress(), legacyInit, "LEGACY");
        await (await proxyDeployer.deployProxy(await tokenImpl.getAddress(), await proxyAdmin.getAddress(), legacyInit, "LEGACY")).wait();
        const legacyToken = await hardhat_1.ethers.getContractAt("HyraToken", legacyProxy);
        return {
            token: legacyToken,
            owner: ownerAddr,
            beneficiary: beneficiaryAddr
    ...
    describe("Security Fix Verification", function () {
        ...
        it("Should demonstrate legacy risk (single holder)", async function () {
            ...
            (0, chai_1.expect)(await legacyToken.balanceOf(fixture.beneficiary.getAddress())).to.equal(INITIAL_SUPPLY);
            // Demonstrate the risk: single holder can transfer immediately
            const transferAmount = hardhat_1.ethers.parseEther("1000000");
            await (0, chai_1.expect)(legacyToken.connect(fixture.beneficiary).transfer("0x9999999999999999999999999999999999999999", transferAmount)).to.not.be.reverted;
            console.log("RISKY: Single holder has immediate access to all tokens");
            console.log(`   - Single holder balance: ${hardhat_1.ethers.formatEther(INITIAL_SUPPLY)} tokens`);
        ...
        it("Should maintain backward compatibility", async function () {
            ...
            (0, chai_1.expect)(await legacyToken.name()).to.equal("Hyra Token Legacy");
            (0, chai_1.expect)(await legacyToken.symbol()).to.equal("HYRA-L");
            (0, chai_1.expect)(await legacyToken.totalSupply()).to.equal(INITIAL_SUPPLY);
