...
describe("HNA-05 Fix: TransparentUpgradeableProxy Compatibility", function () {
    async function deployTestContracts() {
        const [owner, proposer1, executor1, alice, bob] = await hardhat_1.ethers.getSigners();
    ...
    it("should successfully execute upgrade using HyraProxyAdmin", async function () {
        const { timelock, proxyAdmin, token, tokenProxy, newTokenImplementation, proposer1, executor1, owner } = await (0, hardhat_network_helpers_1.loadFixture)(deployTestContracts);
        ...
        const salt = hardhat_1.ethers.keccak256(hardhat_1.ethers.toUtf8Bytes("add-proxy"));
        const delay = 86400;
        await timelock.connect(proposer1)["schedule(address,uint256,bytes,bytes32,bytes32,uint256)"](await proxyAdmin.getAddress(), 0, addData, hardhat_1.ethers.ZeroHash, salt, delay);
        await hardhat_network_helpers_1.time.increase(delay + 1);
        await timelock.connect(executor1)["execute(address,uint256,bytes,bytes32,bytes32)"](await proxyAdmin.getAddress(), 0, addData, hardhat_1.ethers.ZeroHash, salt);
        // Schedule upgrade
        await timelock.connect(proposer1).scheduleUpgrade(await tokenProxy.getAddress(), await newTokenImplementation.getAddress(), "0x", false);
        // Fast forward past delay (7 days)
        await hardhat_network_helpers_1.time.increase(7 * 24 * 60 * 60 + 1);
        // Execute upgrade
        await (0, chai_1.expect)(timelock.connect(executor1).executeUpgrade(await proxyAdmin.getAddress(), await tokenProxy.getAddress())).to.not.be.reverted;
        ...
        (0, chai_1.expect)(currentImplementation).to.equal(await newTokenImplementation.getAddress());
    });
    it("should verify proxy admin is correctly set", async function () {
        const { tokenProxy, proxyAdmin } = await (0, hardhat_network_helpers_1.loadFixture)(deployTestContracts);
        // Check that the proxy admin is correctly set to HyraProxyAdmin
        const currentAdmin = await tokenProxy.admin();
        (0, chai_1.expect)(currentAdmin).to.equal(await proxyAdmin.getAddress());
    });
    it("should verify implementation function works", async function () {
        const { tokenProxy, tokenImplementation } = await (0, hardhat_network_helpers_1.loadFixture)(deployTestContracts);
        // Check that the implementation function returns the correct address
        const currentImplementation = await tokenProxy.implementation();
        (0, chai_1.expect)(currentImplementation).to.equal(await tokenImplementation.getAddress());
    });
    it("should allow upgrade execution via timelock using HyraProxyAdmin", async function () {
        const { timelock, proxyAdmin, tokenProxy, newTokenImplementation, proposer1, executor1 } = await (0, hardhat_network_helpers_1.loadFixture)(deployTestContracts);
        // Schedule upgrade
        await timelock.connect(proposer1).scheduleUpgrade(await tokenProxy.getAddress(), await newTokenImplementation.getAddress(), "0x", false);
        // Fast forward past delay (7 days)
        await hardhat_network_helpers_1.time.increase(7 * 24 * 60 * 60 + 1);
    ...
    it("should reject upgrade calls from non-admin addresses", async function () {
        const { tokenProxy, newTokenImplementation, alice } = await (0, hardhat_network_helpers_1.loadFixture)(deployTestContracts);
        // Try to upgrade from a non-admin address should fail
        await (0, chai_1.expect)(tokenProxy.connect(alice).upgradeToAndCall(await newTokenImplementation.getAddress(), "0x")).to.be.reverted;
    });
    it("should work with HyraProxyDeployer", async function () {
        const { proxyAdmin, alice, bob } = await (0, hardhat_network_helpers_1.loadFixture)(deployTestContracts);
        // Deploy HyraProxyDeployer
        const HyraProxyDeployer = await hardhat_1.ethers.getContractFactory("HyraProxyDeployer");
