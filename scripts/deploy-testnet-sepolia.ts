...
/**
 * Deploy HyraToken to Sepolia Testnet with Production Settings
...
async function main() {
  ...
  console.log("Network: Sepolia Testnet (Production Settings)");
  ...
  console.log("\n3. Deploying HyraToken (Production Settings - 365 days per year)...");
  ...
  const deployment = {
    ...
    type: "Testnet with Production Settings",
  ...
  const deploymentPath = path.join(__dirname, "..", "deployments", `testnet-sepolia-${Date.now()}.json`);
